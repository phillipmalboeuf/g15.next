...
export const Media: FunctionComponent<{
  ...
  return (
    <picture>
      ...
      <img src={cdn(media.fields.file.url) + "?auto=compress,format&w=1800" + (ar ? `&h=${Math.round(ar * 1800)}&fit=fill` : '')} style={ar && { aspectRatio: 1800 / Math.round(ar * 1800) }} alt={`${media.fields.title} ${media.fields.description}`} loading={eager ? "eager" : "lazy"} />
    </picture>
  );
}